...
def under_thousand(number):
    if number in BELOW_10:
        return BELOW_10[number]
    if number == 0:
        return "zero"
    if number == 10:
        return "ten"
    ...
    final = ' '.join(
        f"{BELOW_10[a]}{(' ' + POWERS_OF_10[b]) if POWERS_OF_10[b] else ''}"
        for a, b in n
        if a != 0
    )
    ...
    if matchh:
        ...
        if teen:
            if num == "one":
                new_teen = BELOW_10[11]
            elif num == "two":
                new_teen = BELOW_10[12]
            elif num == "three":
                new_teen = PREFIXES[3] + "teen"
            elif num == "five":
                new_teen = PREFIXES[5] + "teen"
            elif num == "eight":
                new_teen = BELOW_10[8] + "een"
            else:
